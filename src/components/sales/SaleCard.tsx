...
export const SaleCard = ({ sale, index, onDelete }: SaleCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 px-4 py-3.5 bg-surface-800/50 rounded-2xl border border-white/6 active:scale-98 transition-transform"
    >
      {/* Avatar */}
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {sale.montant != null && (
          <span className="text-sm font-semibold text-white">{sale.montant.toFixed(2).replace('.', ',')} €</span>
        )}
        {onDelete && (
          <button
